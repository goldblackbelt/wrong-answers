...
if (supabaseUrl && supabaseServiceKey) {
  ...
  console.log('Service Key 已配置:', !!supabaseServiceKey);
  ...
  console.warn('⚠️ Supabase 配置不完整，使用模拟模式运行');
  console.warn('需要配置 SUPABASE_URL 和 SUPABASE_SERVICE_ROLE_KEY');
}
...
// 创建智能包装器：优先尝试真实 Supabase，失败则用模拟模式
const createSmartClient = (realClient, mockClient) => {
  let useMock = !realClient;
  
  if (realClient) {
    console.log('✅ 已配置真实 Supabase 客户端，将优先使用');
  } else {
    console.warn('⚠️ 未配置真实 Supabase，使用模拟模式');
    useMock = true;
  }
  
  // 包装单个查询对象
  const wrapQueryBuilder = (queryBuilder, tableName) => {
    const wrapped = {};
      ...
      get(target, prop) {
        if (typeof target[prop] === 'function') {
          return (...args) => {
            ...
            // 如果是 single() 或其他终结方法，包装它
            if (prop === 'single' || prop === 'data' || prop === 'error') {
              return result;
            }
  ...
  return {
    from: (table) => {
      if (useMock) {
        console.log(`📦 [${table}] 使用模拟模式`);
        return mockClient.from(table);
      }
      
      try {
        const realQuery = realClient.from(table);
        console.log(`🚀 [${table}] 使用真实 Supabase`);
        return realQuery;
      } catch (error) {
        console.error(`❌ [${table}] Supabase 失败，回退到模拟模式:`, error);
        useMock = true;
        return mockClient.from(table);
      }
    }
...
if (supabase) {
  finalClient = createSmartClient(supabase, mockClient);
} else {
  finalClient = mockClient;
}

// 导出Supabase客户端供其他模块使用
global.supabase = finalClient;
